...
import { startXucXacWatcher, resetXucXacWatcher, buildXucXacStatsMsg, prependToXucXacCache } from "../analyzer/xucxac";
...
export function createBot(token: string) {
  ...
  async function tryStartXucXacRealtime() {
    const started = await startXucXacWatcher(async (session) => {
      ...
      // Cập nhật cache realtime — không cần re-fetch 1000 phiên
      prependToXucXacCache(session);
