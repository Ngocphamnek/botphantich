...
export async function fetchAndBuildAdvanced(key: string): Promise<AdvancedPrediction> {
  ...
  try {
    ...
    if (key === "xucxac") {
      const { getCachedXucXacSessions } = await import("./xucxac");
      // Dùng cache 1000 phiên — không fetch lại Telegram mỗi lần
      const xucSessions = await getCachedXucXacSessions();
    ...
    // Tournament là nguồn duy nhất — luôn dùng champion kể cả khi confidence thấp
    const result: AdvancedPrediction = {
      action: tour.action,
      prediction: tour.prediction,
      confidence: tour.confidence,
      trendScore: tour.confidence,
      freqScore: 0,
      revScore: 0,
      reason: tour.action === "BET"
        ? `Tournament champion: ${tour.champion} (${(tour.accuracy * 100).toFixed(1)}% · ${tour.testedSessions}p backtest · ${tour.roundsRun} vòng loại)`
        : `Tournament chưa đủ tin cậy — champion ${tour.champion} (${(tour.accuracy * 100).toFixed(1)}%)`,
      indicators: `${tour.champion} thắng ${tour.roundsRun} vòng loại trong ${tour.testedSessions} phiên`,
      capitalAdvice: tour.accuracy >= 0.65
        ? "Vốn vừa (15–20% bankroll)"
        : tour.accuracy >= 0.55
        ? "Vốn nhỏ (5–10% bankroll)"
        : "⚠️ Chưa đủ độ tin cậy — cân nhắc bỏ qua",
      counterN: null, counterResult: null,
...
export async function analyzeGame(key: string): Promise<string> {
  ...
  if (key === "xucxac") {
    try {
      const { getCachedXucXacSessions, buildXucXacAnalysis } = await import("./xucxac");
      const sessions = await getCachedXucXacSessions();
